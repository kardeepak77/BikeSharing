...
boostFit <- train(cnt ~ weathersit + workingday + hum*atemp + yr, data = bikeSharingTrainData, 
                method = "gbm",
                trControl=trainControl(method = "repeatedcv", number = 10),
                preProcess = c("center", "scale"),
...
#MLR2 Fit
mlrFit2 <- lm(cnt ~ weathersit + season + I(atemp)^2 + I(hum)^2 + I(windspeed) + yr, data=bikeSharingTrainData)
